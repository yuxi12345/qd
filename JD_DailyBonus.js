...
1签到脚本将在每天的凌晨0:05执行, 您可以修改执行时间。 因部分接口京豆限量领取, 建议调整为凌晨签到。
...
function ReadCookie() {
  ...
  if (DeleteCookie) {
    ...
    $nobyda.notify("脚本终止", "", '未关闭脚本内"DeleteCookie"选项 ??')
    $nobyda.done()
    return
  ...
  if (KEY || OtherKey) {
    ...
    $nobyda.notify("京东签到", "", "脚本终止, 未获取Cookie ??")
    $nobyda.done()
  }
}
        ...
        try {
          ...
          if (data.match(/\"resBusiCode\":0/)) {
            ...
            if (data.match(/已经领取|\"resBusiCode\":15/)) {
              ...
              merge.JRSteel.notify = "京东金融-钢镚: 失败, 原因: 未知 ??"
            }
          }
        } catch (eor) {
      ...
      $nobyda.get({
        ...
        try {
          ...
          if (cc.status && cc.data && cc.data.firstLoginStatus) {
            ...
            merge.JDBuyCar.notify = `京东商城-汽车: 失败, ${qt}${cc.error&&cc.error.code==2000?`??`:` ??`}`
      ...
      $nobyda.post({
        ...
        try {
          ...
          if (cc.status === true) {
            ...
            console.log(`\n京东商城-汽车签到失败 ${Details}`);
            merge.JDBuyCar.fail = 1;
            merge.JDBuyCar.notify = `京东商城-汽车: 失败, ${cc.error&&cc.error.msg?cc.error.msg:`原因: 未知`} ??`;
          }
        } catch (eor) {
    ...
    $nobyda.get({
      ...
      try {
        ...
        if (cc.msg == 'success' && cc.retcode == 0) {
          ...
          merge.TotalBean.nickname = cc.retcode == 1001 ? "Cookie失效 ??" : "";
          console.log(`\n京东-总京豆查询失败 ${Details}`)
        }
      } catch (eor) {
...
function GetCookie() {
  try {
    if ($request.method != 'OPTIONS' && $request.headers && $request.url !== 'http://www.apple.com/') {
      ...
      if (!acObj.cookie) {
        $nobyda.notify("写入京东Cookie失败", "", "请查看脚本内说明, 登录网页获取 ??")
        return
      } else {
        ...
        if (status) {
          if (status > 0) {
            ...
            $nobyda.notify(`用户名: ${acObj.userName}`, ``, `${status==2?`更新`:`写入`}京东 [账号${seatNo+1}] Cookie${WT?`成功 ??`:`失败 ??`}`)
          } else {
            console.log(`\n用户名: ${acObj.userName}\n与历史京东 [账号${seatNo+1}] Cookie相同, 跳过写入 ??`)
          }
        }
      }
    } else if ($request.url === 'http://www.apple.com/') {
      $nobyda.notify("京东签到", "", "类型错误, 手动运行请选择上下文环境为Cron ??");
    } else {
      $nobyda.notify("京东签到", "写入Cookie失败", "请检查匹配URL或配置内脚本类型 ??");
    }
  } catch (eor) {
